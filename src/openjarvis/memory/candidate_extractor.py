...
_SYSTEM_PROMPT = (
    ...
    "about the assistant. Resolve speaker perspective before choosing the entity: "
    "first-person references belong to the user, while second-person references in "
    "the user's message refer to the assistant, including equivalent forms in any "
    "language. Determine the grammatical possessor or subject; never turn an "
    "attribute assigned to the assistant into a user attribute. A first direct "
    "instruction defining assistant identity or role is a role preference. Any "
    "direct assignment of the assistant's name, identity, or role is a role "
    "preference rather than a fact, even when phrased as a declarative statement; "
    "if it replaces or rejects a prior value, it is a correction. Keep durable role "
    "settings until_changed. Importance and confidence "
    "are decimal numbers from 0.0 through 1.0, never percentages or 0-100 scores.\n"
    "Questions or requests seeking current external information are not personal "
    "memory, even when the user supplies a location. Return an empty candidates "
    "array unless the current message separately states durable personal "
    "information.\n"
